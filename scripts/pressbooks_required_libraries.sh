...
if [ "$ARCHITECTURE" ]; then
	if [ "$ARCHITECTURE" == amd64 ]; then
		...
		wget https://www.princexml.com/download/prince_15.4.1-1_debian11_arm64.deb
		gdebi prince_15.4.1-1_debian11_arm64.deb --n
		rm prince_15.4.1-1_debian11_arm64.deb
	else
		wget https://www.princexml.com/download/prince-15.4.1-linux-generic-aarch64.tar.gz
    tar -xvzf prince-15.4.1-linux-generic-aarch64.tar.gz
    rm prince-15.4.1-linux-generic-aarch64.tar.gz
    cd prince-15.4.1-linux-generic-aarch64
    echo "" | ./install.sh /usr # install to /usr automatically
    cd ..
    rm -rf prince-15.4.1-linux-generic-aarch64
	fi
else
